...
public class QuestNavSubsystem extends SubsystemBase {
  ...
  private Transform3d ROBOT_TO_QUEST = new Transform3d(Constants.ROBOT_TO_QUEST.getX(), Constants.ROBOT_TO_QUEST.getY(),
      Constants.ROBOT_TO_QUEST.getZ(), Constants.ROBOT_TO_QUEST.getRotation());
  ...
  public QuestNavSubsystem(Drivebase drivebase) {
    ...
    // There is a bug with the 2026-1.0.0 version of the questnav library which
    // caused the version check to fail and span the console every 10 seconds
  ...
  public void resetToZeroPose() {
    ...
    // Because the pose is set on the quest nav we will need to store an offset that
    // is updated by the limelight in the drivebase class because we do not want to
    // send constant updates to the questnav system
    // questNav.setPose(questPose3d);
    // System.out.println("QuestNav internal pose reset to: " +
    // questPose3d.toString());
  }

  public Pose3d getQuestRobotPose() {
    return (poseFrames != null && poseFrames.length > 0) ? poseFrames[poseFrames.length - 1].questPose3d()
        .transformBy(ROBOT_TO_QUEST.inverse()) : nullPose;
  }

  public double getQTimeStamp() {
    return (poseFrames != null && poseFrames.length > 0) ? poseFrames[poseFrames.length - 1].dataTimestamp() : 0;
  }

  public double getQAppTimeStamp() {
    return (poseFrames != null && poseFrames.length > 0) ? poseFrames[poseFrames.length - 1].appTimestamp() : 0;
  }

  public Pose3d getQuestPose() {
    return (poseFrames != null && poseFrames.length > 0) ? poseFrames[poseFrames.length - 1].questPose3d() : nullPose;
  }
  ...
  public void periodic() {
    // Make these smartDashboard stuff send less data, maybe it is smart to not send
    // data it does not need to
    if (questNav.isConnected()) {
      hasQuest = true;
      // If the x or y difference from the robots current pose to the limelight
      // estimate pose update the current quest estimate for the position
      ...
      if (System.currentTimeMillis() - this.lastResetTime > 1500) {
        if (RobotContainer.visionSubsystem.frontLimelightSee || RobotContainer.visionSubsystem.rightLimelightSee) { // One
                                                                                                                    // of
                                                                                                                    // the
                                                                                                                    // limelight
                                                                                                                    // must
                                                                                                                    ...
                                                                                                                    // tags
          if (Math
              .abs(drivebase.getPose().getX()
                  - drivebase.limelightPoseEstimate.getX()) > Constants.LIMELIGHT_QUEST_ERROR_AMOUNT_METERS
              || Math.abs(drivebase.getPose().getX()
                  - drivebase.limelightPoseEstimate.getY()) > Constants.LIMELIGHT_QUEST_ERROR_AMOUNT_METERS) {
